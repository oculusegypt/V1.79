...
export function SuggestionCards({ suggestions, loading, onSelect, onDismiss }: {
  suggestions?: string[];
  loading?: boolean;
      ...
      {loading ? (
        ...
        <div className="flex flex-wrap gap-2 items-center">
          {suggestions!.map((q, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => onSelect(q)}
                className="text-right text-[10.5px] px-3 py-1.5 rounded-full transition-all active:scale-95 leading-snug font-semibold relative group"
                style={isContinueBtn
          ...
          {onDismiss && (
            <button
              onClick={onDismiss}
              className="p-1.5 rounded-full text-muted-foreground/40 hover:text-muted-foreground hover:bg-muted/50 transition-all"
              aria-label="تجاهل الاقتراحات"
            >
              <X size={14} />
            </button>
          )}
