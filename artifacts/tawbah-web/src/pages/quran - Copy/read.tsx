...
import { preloadQuranVerseFast as preloadQuranVerse, getCachedAudioUrl, getAudioUrlDirect, type QuranAudioSource } from "@/lib/quran-audio";
...
function MushafReader({ surah, reciterId, onBack }: { surah: Surah; reciterId: string; onBack: () => void }) {
  ...
  const playIdx = useCallback(async (idx: number) => {
    ...
    const tryHtmlAudio = async () => {
      ...
      audio.onended = () => {
        const next = idx + 1;
        if (next < ayahs.length) playIdxRef.current(next);
